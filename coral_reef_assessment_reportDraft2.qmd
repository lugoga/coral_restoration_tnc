  ...
  pdf:
    ...
    include-in-header:
      text: |
        ...
        \pretitle{\begin{center}\LARGE\bfseries\includegraphics[width=8cm]{the-nature-conservancy.png}\\[2em]}
...
This report presents findings from a systematic multi-metric reef health assessment conducted at seven sites across two regions: **Unguja North** (Chama Kangu, Mwamba Mrefu, Mayale, and Chumani) and **Mtwara** (Chamba Cha Kati, Nusura, and Viulu). The assessment evaluated four ecological metrics --- benthic substrate composition, fish density, coral recruit density, and invertebrate assemblages --- to: (i) characterise reef condition at each site; (ii) compare condition between MMA and non-MMA sites; and (iii) identify priority areas for active restoration intervention under the *Bahari Yetu* project.
...
# site_sf |> st_write('data/site_locations.gpkg', append = FALSE)
...
be_ung |> 
  ...
  flextable::colformat_double(j = 5:6, digits = 5) |> 
...
Detailed percentage cover for specific substrate categories in Mtwara is provided in @tbl-substrate-mtwara. **Chamba Cha Kati**, located within the Mnazi Bay Marine Managed Area (MMA), displayed exceptional reef health with a hard coral cover of 80.8% and a low prevalence of fleshy algae (7.5%). This high coral-to-algae ratio suggests a resilient ecosystem state, likely contributed by the long-term management and enforcement within the Mnazi Bay–Ruvuma Estuary Marine Park (MBREMP). 
...
be_mt |>
 ...
 ungroup() |> 
  ...
  flextable::flextable() |>
  flextable::set_header_labels(substrate_category = "Substrate") |>
  flextable::add_header_row(values = c("", "Percentage Cover"), colwidths = c(1, 3)) |>
  flextable::colformat_double(digits = 1, na_str = "-") |>
  ...
  flextable::color(part = "header", color = "white", i = 2) |>
  flextable::fontsize(size = 11, part = "all") |>
  flextable::bg(j = 2:4, bg = scales::col_quantile(palette = "Reds", domain = c(0,100), na.color = "#cfceceff"), part = "body") |>
  flextable::autofit()

```

In contrast, sites situated outside the MBREMP showed varying degrees of composition (@fig-substrate-mtwara); **Nusura** and **Viulu** recorded 65.8% and 54.9% hard coral cover respectively, with Viulu exhibiting a higher dominance of fleshy algae (26.9%) and soft coral (13.1%). 
...
Fish assemblages in Mtwara were characterized by high spatial variability in density and family composition (@fig-fish-biomass-mtwara-error). **Chamba Cha Kati** (Mnazi Bay) exhibited the highest overall fish densities, dominated by Pomacentridae (254.6 individuals per 100 m^2^), which was nearly ten times higher than densities recorded at **Nusura** (26.0) and **Viulu** (22.0) (@tbl-fish-biomass-mtwara). 

Herbivorous families such as Scaridae and Acanthuridae were consistently present across all sites, with the highest Scaridae density at Chamba Cha Kati (11.0). In contrast, **Viulu** supported higher densities of Siganidae (4.4) and Mullidae (5.0) compared to other sites. While Chamba Cha Kati was unique in hosting families like Holocentridae and Balistidae, the non-MMA sites Nusura and Viulu supported higher densities of Lethrinidae and Pomacanthidae. These results indicate that while the protected Chamba Cha Kati reef maintains superior biomass and damselfish abundance, the surrounding reefs still support diverse functional groups essential for reef resilience.
...
Coral recruitment in Mtwara was characterized by high taxonomic diversity and spatial variability across the three sites (@fig-recruit-mtwara-error). **Chamba Cha Kati** (Mnazi Bay) exhibited strong recruitment of framework-building genera, particularly *Acropora* and *Merulina* (both at 500 ind. 100 m^-2^), alongside significant densities of *Alveopora*, *Lobophyllia*, and *Porites* (300 ind. 100 m^-2^ each). In contrast, **Nusura** was dominated by massive *Porites* recruits (650 ind. 100 m^-2^), while **Viulu** showed the highest recruitment of *Alveopora*, *Favia*, and *Astreopora* (400 ind. 100 m^-2^ each) (@tbl-recruit-mtwara). The presence of specialized genera such as *Gardineroseris* and *Plerogyra* exclusively at Chamba Cha Kati, combined with high *Acropora* density, indicates a robust recovery potential within the MMA compared to the more opportunistic or stress-tolerant assemblages observed at the other sites.
...
filter(density_100m2 > 0) |> 
  ...
  flextable::flextable() |> 
  flextable::set_header_labels(family = "Family") |> 
  flextable::add_header_row(values = c("", "Recruit Density (individuals per 100m2)"), colwidths = c(1, 3)) |> 
  flextable::colformat_double(digits = 0, na_str = "-") |> 
  flextable::align(j = 2:4, align = "center", part = "all") |> 
  flextable::bold(part = "header") |> 
  flextable::bg(part = "header", bg = "#004763", i = 2) |> 
  flextable::color(part = "header", color = "white", i = 2) |> 
  flextable::fontsize(size = 11, part = "all") |> 
  flextable::bg(j = 2:4, bg = scales::col_quantile(palette = "Reds", domain = c(100,600), na.color = "#cfceceff"), part = "body") |> 
  flextable::autofit()
...
Indicator invertebrate densities in Mtwara varied significantly across the three surveyed reefs (@tbl-invertebrate-mtwara1). **Viulu** exhibited the highest overall densities for several key groups, including sea stars (900 ind. 100 m^-2^), urchins (1,150 ind. 100 m^-2^), and gastropods (800 ind. 100 m^-2^). **Nusura** showed moderate densities, notably supporting the highest recorded density of sea cucumbers (300 ind. 100 m^-2^). In contrast, **Chamba Cha Kati** generally recorded lower invertebrate abundances, although it maintained stable populations of bivalves (300 ind. 100 m^-2^) and octopuses (100 ind. 100 m^-2^). Notably, Crown-of-Thorns starfish (COTs), a significant coral predator, were recorded exclusively at **Viulu** (200 ind. 100 m^-2^, *See* @fig-inverts-mtwara), representing a localized threat to reef recovery.
...
filter(density_100m2 > 0) |> 
  ...
  flextable::flextable() |> 
  flextable::set_header_labels(family = "Family") |> 
  flextable::add_header_row(values = c("", "Invertebrates (individuals per 100m2)"), colwidths = c(1, 3)) |> 
  flextable::colformat_double(digits = 0, na_str = "-") |> 
  flextable::align(j = 2:4, align = "center", part = "all") |> 
  flextable::bold(part = "header") |> 
  flextable::bg(part = "header", bg = "#004763", i = 2) |> 
  flextable::color(part = "header", color = "white", i = 2) |> 
  flextable::fontsize(size = 11, part = "all") |> 
  flextable::bg(j = 2:4, bg = scales::col_quantile(palette = "Reds", domain = c(100,1500), na.color = "#cfceceff"), part = "body") |> 
  flextable::autofit()
...
condition_df <- tibble(
  site      = c("Chumani", "Mayale", "Chama Kangu", "Mwamba Mrefu", 
                "Chamba Cha Kati", "Nusura", "Viulu"),
  region    = c(rep("Unguja North", 4), rep("Mtwara", 3)),
  gov_score = c(75, 70, 40, 45, 95, 55, 50),
  eco_score = c(65, 58, 35, 30, 92, 68, 60),
  mma       = c(TRUE, TRUE, FALSE, FALSE, TRUE, FALSE, FALSE)
)
...
Sites falling in the lower-left quadrant --- **Chama Kangu** and **Mwamba Mrefu** --- represent the highest restoration priority. Despite high recruit densities at Chama Kangu, the extreme algal dominance (>75%) and low coral cover indicate a persistent phase shift that requires active intervention. **Chamba Cha Kati** (Mnazi Bay) remains the regional benchmark for reef health, exhibiting the highest coral cover (80.8%) and fish biomass. **Nusura** and **Viulu** show moderate-to-high ecological resilience but lack formal protection, making them prime candidates for MMA establishment to safeguard their existing coral framework.

## MMA Effectiveness {#sec-mmaeffect}

\index{marine managed areas (MMAs)!effectiveness}

The assessment confirms that formal management significantly correlates with reef health. **Chamba Cha Kati** (Inside MBREMP) and **Chumani/Mayale** (Inside MCA) maintain higher coral-to-algae ratios compared to open-access sites. However, the high fish density at **Chama Kangu** (non-MMA) suggests that even degraded benthos can support high abundances of certain functional groups (e.g., Haemulidae), though these lack the biomass of larger predators found in protected zones. The presence of COTs at **Viulu** highlights that even relatively healthy non-MMA sites face acute biological threats that require managed responses.
...
restoration_table <- tibble(
  ...
  Site      = c("Chama Kangu", "Mwamba Mrefu", "Viulu", "Nusura", 
                "Mayale", "Chumani", "Chamba Cha Kati"),
  Region    = c("Unguja North", "Unguja North", "Mtwara", "Mtwara", 
                "Unguja North", "Unguja North", "Mtwara"),
  `Key Issues` = c(
    "Extreme algal dominance (76%); low coral cover (5%)",
    "High macroalgae; low coral cover (8.8%); low fish density",
    "Presence of COTs; high algal cover (26.9%); no formal protection",
    "High coral cover but lacks formal management; sea urchin dominance",
    "High sand cover; high urchin density (282 ind/100m2)",
    "Moderate coral cover; low invertebrate diversity",
    "Excellent condition (80% coral); high fish biomass"
  ),
  `Recommended Actions` = c(
    "Active macroalgae removal; coral outplanting; MMA establishment",
...
This assessment provides a systematic multi-metric evaluation of reef health across Unguja North and Mtwara, establishing a baseline for the *Bahari Yetu* project. Key findings are as follows:

1. **Regional Benchmarks:** **Chamba Cha Kati** (Mnazi Bay) represents the healthiest reef assessed, with over 80% hard coral cover and superior fish densities, serving as a critical reference site and potential larval source for regional recovery.

2. **Degradation Hotspots:** **Chama Kangu** and **Mwamba Mrefu** in Unguja North are in a state of ecological crisis, characterized by severe phase shifts toward algal dominance (up to 76%) and critically low hard coral cover (<10%). These sites require urgent active restoration, including macroalgae removal and coral outplanting.

3. **Management Impact:** The study confirms that formal protection within MMAs (e.g., MBREMP and North Unguja MCAs) significantly correlates with higher coral-to-algae ratios and fish biomass. However, high-quality reefs like **Nusura** and **Viulu** currently lack formal protection, making them vulnerable to future degradation.

4. **Trophic Imbalance:** While fish densities are relatively stable, the severe depletion of high-value invertebrates like sea cucumbers and the localized presence of Crown-of-Thorns starfish at **Viulu** indicate significant harvesting pressure and emerging biological threats that require targeted management.

5. **Restoration Strategy:** Restoration must be tiered; immediate active intervention is needed for degraded Unguja sites, while Mtwara efforts should focus on formalizing governance for healthy "outside" reefs and implementing biological controls (e.g., COTs removal) to maintain existing resilience.
